...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--socket-port", type=int, default=8766)
...
def main(argv=None) -> int:
    ...
    config = get_config(); data_root = args.data_root or config["save_folder"]
    backup_root = args.backup_root or config["backup_folder"]
    repository = Path(__file__).parents[1]
    weather_locations = args.weather_locations or str(repository / "extractor_modules" / "weather" / "owm_locations.txt")
    cctv_locations = args.cctv_locations or str(repository / "extractor_modules" / "cctv" / "cctv.kml")
    catalog = DataCatalog(data_root, backup_root); sources = args.source or sorted(READERS)
    seen: set[str] = set()
    sink = (SocketJSONLSink(args.socket_host, args.socket_port, timeout=args.ack_timeout, retries=args.network_retries)
            if args.socket_host else JSONLSink(sys.stdout))
    emit = sink.write
    try:
        if args.start != "now":
